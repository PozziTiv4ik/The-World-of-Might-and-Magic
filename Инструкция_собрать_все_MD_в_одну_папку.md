...
Важно: папка `Все_MD_файлы` является временным экспортом и добавлена в `.gitignore`. Не используй ее как источник канона и не учитывай ее при обычной работе ИИ, если пользователь прямо не попросил собрать плоский архив.
